...
static int UserDataGC(lua_State* HolyWater) {
	void* UD = lua_touserdata(HolyWater, 1);
	if (Bridge::m_rL) {

		r_lua_rawgeti(Bridge::m_rL, LUA_REGISTRYINDEX, (int)UD);
		if (r_lua_type(Bridge::m_rL, -1) <= R_LUA_TNIL) {
			lua_pushnil(HolyWater);
			lua_rawseti(HolyWater, LUA_REGISTRYINDEX, (int)UD);
...
void Executeee(std::string Script) {
	...
	UserDataGC(m_L); // like garbage collector but not like garbage collector
	GarbageCollector(m_L);//prevents or hopefully prevents memleaks (memleaks are bad memleaks crash game memleaks no good)
...
DWORD WINAPI input(PVOID lvpParameter)
{
	...
	char buffer[999999];
	DWORD dwRead;
	hPipe = CreateNamedPipe(TEXT("\\\\.\\pipe\\RAGE"),
...
void GetRobloxLuaTypes() {
	...
	r_lua_getfield(m_rL, -1, "Gravity"); //stephen hawking
...
void InitGlobals()
{
	...
	printf(" - RAGE is locked and loaded!\n");
}
